...
# Allows the separate Jenkins publisher to upload development images to the
# same repository without changing the existing GitHub publisher grant.

resource "google_artifact_registry_repository_iam_member" "jenkins_publisher_writer" {
  project    = var.project_id
  location   = google_artifact_registry_repository.application.location
  repository = google_artifact_registry_repository.application.repository_id
  role       = "roles/artifactregistry.writer"
  member     = "serviceAccount:${google_service_account.jenkins_publisher.email}"
}
